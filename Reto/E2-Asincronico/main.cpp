...
void TrafficLight(float xPos, float zPos, int LightColor){
  glPushMatrix();
...
void TrafficLightInv(float xPos, float zPos, int LightColor){
  glPushMatrix();
  glTranslatef(xPos,0,zPos);
  if(LightColor == 1)
  {
...
int LightControl2(int &LightCTRL2){
  if(LightCTRL2 == 200){
    LightCTRL2 = 0;
    LightColor2 ++;
  }
...
// La proxima posision se calcula asi:
// Position[0] += speed * Direction[0];
// Position[2] += speed * Direction[2];
bool checkCollision(int initialIndex)
{
  float xPos1 = Agents[initialIndex].Position[0] + speed * 2 * Agents[initialIndex].Direction[0];
  float zPos1 = Agents[initialIndex].Position[2] + speed * 2 * Agents[initialIndex].Direction[2];
  float distance, xDiff, zDiff, xPos2, zPos2;

  for (int i = 0; i < NumberOfAgents; i++)
  {
    if (i != initialIndex)
    {
      xPos2 = Agents[i].Position[0];
      zPos2 = Agents[i].Position[2];
...
// Nodo 5, 6 y 31
bool checkStop(float xPos, float yPos)
{
  if (LightColor == 0)
  {
    if ((xPos >= -15 && xPos <= -10) && (yPos >= 40 && yPos <= 45))
    {
      return true;
    }

    if ((xPos >= 70 && xPos <= 75) && (yPos >= 10 && yPos <= 15))
    {
      return true;
    }
  ...
  if (LightColor == 1)
  {
    if ((xPos >= 15 && xPos <= 20) && (yPos >= -15 && yPos <= -10))
    {
      return true;
    }

    if ((xPos >= 54 && xPos <= 58) && (yPos >= 72 && yPos <= 76))
    {
      return true;
    }
  ...
  if (LightColor2 == 0)
  {
    if ((xPos >= -2 && xPos <= 3) && (yPos >= -151 && yPos <= -148))
    {
      return true;
    }

    if ((xPos >= 74 && xPos <= 78) && (yPos >= -177 && yPos <= -174))
    {
      return true;
    }
  ...
  if (LightColor2 == 1)
  {
    if ((xPos >= 49 && xPos <= 53) && (yPos >= -118 && yPos <= -114))
    {
      return true;
    }
  }
...
void display(){
  ...
  TrafficLight(35, 20, LightColor);
  TrafficLight(45, 20, LightColor);
  TrafficLightInv(40, 10, LightColor);
  TrafficLightInv(40, 30, LightColor);

  TrafficLight(50, -175, LightColor2);
  TrafficLight(40, -175, LightColor2);
  TrafficLightInv(45, -165, LightColor2);

  LightColor = LightControl(LightCTRL);
  LightColor2 = LightControl2(LightCTRL2);
